...
const shopifyClient = new GraphQLClient(`https://${MYSHOPIFY_DOMAIN}/admin/api/2023-07/graphql.json`, {
    headers: {
...
server.tool("create-fulfillment", {
    orderId: z.string().min(1),
    trackingInfo: z
        .object({
...
server.tool("get-inventory-levels", {
    locationId: z.string().optional(),
    limit: z.number().default(10)
}, async (args) => {
...
server.tool("get-inventory-items", {
...
server.tool("set-inventory-tracking", {
    inventoryItemId: z.string().min(1, "Inventory item ID is required"),
    tracked: z.boolean()
}, async (args) => {
...
server.tool("connect-inventory-to-location", {
    inventoryItemId: z.string().min(1, "Inventory item ID is required"),
    locationId: z.string().min(1, "Location ID is required"),
...
server.tool("disconnect-inventory-from-location", {
    inventoryItemId: z.string().min(1, "Inventory item ID is required"),
    locationId: z.string().min(1, "Location ID is required"),
    reason: z.string().optional()
}, async (args) => {
