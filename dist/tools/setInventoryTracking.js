    ...
    execute: async (input) => {
      ...
      `;
            ...
            if (data.inventoryItemUpdate.userErrors.length > 0) {
                throw new Error(`Failed to set inventory tracking: ${data.inventoryItemUpdate.userErrors
            ...
            const inventoryLevels = inventoryItem.inventoryLevels.edges.map((edge) => {
            ...
            throw new Error(`Failed to set inventory tracking: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
};
export { setInventoryTracking };
//# sourceMappingURL=setInventoryTracking.js.map