...
const createProduct = {
    ...
    execute: async (input) => {
        try {
            ...
            const collectionsToJoin = input.collectionsToJoin?.map(id => id.startsWith("gid://") ? id : `gid://shopify/Collection/${id}`);
            const query = gql `
      ...
      `;
            ...
            // Format metafields
            const metafields = product.metafields.edges.map((metafieldEdge) => {
